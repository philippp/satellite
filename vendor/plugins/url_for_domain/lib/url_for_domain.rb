...
module ActionController
  class UrlRewriter
    def rewrite(options = {})
      #rails2 occasionally passed in nil as the value for options
      #suck it
      #recreate by removeing this line, do a render to string (photos app)
      # rails 2.0.2
      options ||= {}
      
      unless !options.nil? and options[:subdomain].nil?
        if !options[:domain].nil? and options[:subdomain] == false
          newhost = options[:domain]
