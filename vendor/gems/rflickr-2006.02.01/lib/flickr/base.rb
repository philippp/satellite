...
############################################################################
        ...
        def call_unauth_method(method,args={})
                ...
                args.each{|k,v| args[k] = args[k].to_s }
                begin
                        tries -= 1;
                ...
                rescue DRb::DRbConnError => drbe
                        $stderr.puts "Connection error, will try #{tries} more times."
                        if tries > 0
                                retry
                        else
                                raise drbe
                        ...
                        $stderr.puts "Socket error, will try #{tries} more times."
                        if tries > 0
                                retry
                        else
                                raise se
...
class Flickr::PhotoSet < Array
        ...
        def primary() @primary ||= @flickr.photos.getInfo(@primary_id.to_s) end
end
